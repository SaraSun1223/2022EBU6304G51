...
public class ExtraSeat extends MouseAdapter{
	...
	public void Seat2(JPanel t2){
		...
		for (int i = 0; i <num; i++) {
			...
			btns[i].setBackground(new Color(210,118,92));
			btns[i].setForeground(Color.white);
			btns[i].setOpaque(true);
			btns[i].setFont(new Font("",Font.BOLD,12));
			...
			if(e.getSource()==btns[i]){
				if(state==-1){
					...
					if(seatcontrol.getStatusBySeat(btns[i].getText()).getStatus()){
						...
						btns[state].setBackground(new Color(210,118,92));
						btns[state].setBorderPainted(false);
						state = -1;
						option = "";
					}
				}
